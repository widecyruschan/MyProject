...
class shopping{
	...
	public static function get_cart_sid($array = true){
		$ids = trim(preg_replace('/[^\d,]/','',$_COOKIE['MYCMS_cart_sid']),','); 
	...
	public static function del_cart($id = 0,$empty = false){
		if($empty){
			global $gmt_time;
			setcookie('MYCMS_cart_sid','',($gmt_time - 3600),'/');
		} elseif(is_numeric($id)) {
			$sid = self::get_cart_sid(true);
			$sid = array_diff($sid,array($id));
			setcookie('MYCMS_cart_sid',implode(',',$sid),0,'/');
		}
		return true;
	}
	...
	public static function del_order($id = 0,$cart = ""){
		...
		if($db->execute($sql)){
			empty($cart) or setcookie('MYCMS_cart_sid',$cart,0,'/');
			return true;	
		} else {
			return false;
		}		
