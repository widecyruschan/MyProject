...
class article{
	public static function data(){
		...
		foreach($result as &$n){
			if(isset($n['id'])){ 
				...
				$db1=new db();
				$GetUPchannleID=$db1->query("SELECT id FROM `cms_channel` where `id`=(SELECT `UpchannelID` FROM `cms_channel` where `id`={$n['channel']} )",1);
				...
				$vs=$db1->query("SELECT * FROM `cms_fields` WHERE ".$fieldswhere." order by  fieldorder DESC,fid");
				if($vs){
					$db2=new db();
					$c=$db2->query("SELECT * FROM `cms_article_field` WHERE aid=".$n['id'],1);
				foreach($vs as $f){
	...
	private static function single($id,$related = 0){
		...
		$GetUPchannleID=$db1->query("SELECT id FROM `cms_channel` where `id`=(SELECT `UpchannelID` FROM `cms_channel` where `id`={$ChannleID} )",1);
		if($GetUPchannleID['id']!=0){
