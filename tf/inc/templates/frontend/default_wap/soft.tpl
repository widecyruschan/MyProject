...
<!-- 有子类时显示子类 -->
{:$sub = category::data(4,0,$mycms['category']['id'],$mycms['category']['level']+1):}
...
<div class="box">
	<div class="main">
		<div class="titlebar">
			...
			<ol class="location">{:$lang['page']['location']:}<a href="./">{:$lang['page']['home']:}</a> &gt;&gt; <a href="{:misc::get_url('channel',4,$mycms['channel_id']):}">{:$mycms['category']['channel']:}</a> &gt;&gt; {:category::cid2name($mycms['category']['nexus'],' &gt;&gt; ',true):}</ol>
		</div>
		
		<!-- 列表数据，全部分类 -->
		{:foreach download::data(0,10,0,0,$mycms['category']['id'],0,0,0,0,1,0,0,3) as $d:}
		<div class="data">
			<ul class="image"><li><a href="{:$d['URL']:}" title="{:$d['title']:}"><img src="{:$d['picture'][0]:}" alt="{:$d['title']:}" width="135" height="90" border="0" onload="mycms.img_auto_size(this)" /></a></li></ul>
			<ul class="content">
	...
	<div class="sidebar">
		...
		<div class="well">
			{:foreach download::data(0,10,1,$mycms['category']['channel_id']) as $i => $d:}
			<ul>
				<li class="num">{:$i + 1:}</li>
		...
		<div class="well">
			{:foreach download::data(9,10,0,$mycms['category']['channel_id']) as $i => $d:}
			<ul>
				<li class="num">{:$i + 1:}</li>
