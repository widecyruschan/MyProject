...
define('XML_INTERFACE','http://www.CyrusChanHK.com/_client/template/');
$mycms['type'] = $type < 1 ? 1 : ($type > 3 ? 3 : $type);
switch ($mycms['type']){
	case 1 :
		$xml_dir = 'xml_free'; 
		break;
...
if(is_numeric($total_page)){
	...
	empty($template_list) && $mycms['xml_status'] = false;
} else {
	$mycms['xml_status'] = false;
}
$mycms['template_list'] = $template_list;
$mycms['page'] = page($page,$total_page);
